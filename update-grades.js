        ...
        for (let i = 2; i < rows.length; i++) {
            ...
            if (row.c && row.c.length >= 4) {
                ...
                if (cedula && cedula !== '0' && apellidos && nombres) {
                    const evaluaciones = [
                        { nombre: 'Exposiciones', puntaje: parseFloat(row.c[4]?.v) || 0, ponderado: 0, porcentaje: 20 },
                        { nombre: 'Ejercicios Exposición', puntaje: parseFloat(row.c[5]?.v) || 0, ponderado: 0, porcentaje: 20 },
                        { nombre: 'Informes Acumulados', puntaje: parseFloat(row.c[6]?.v) || 0, ponderado: 0, porcentaje: 20 },
                        { nombre: 'Ejercicios Acumulados', puntaje: parseFloat(row.c[7]?.v) || 0, ponderado: 0, porcentaje: 20 },
                        { nombre: 'Asistencia y Participación', puntaje: parseFloat(row.c[8]?.v) || 0, ponderado: 0, porcentaje: 20 }
                    ];
